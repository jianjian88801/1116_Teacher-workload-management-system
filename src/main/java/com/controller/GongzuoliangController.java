    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(gongzuoliang !=null){
            ...
            BeanUtils.copyProperties( gongzuoliang , view );//把实体数据重构到view中
                ...
                //级联表
                JiaoshiEntity jiaoshi = jiaoshiService.selectById(gongzuoliang.getJiaoshiId());
                if(jiaoshi != null){
    ...
    */
    ...
    public R save(@RequestBody GongzuoliangEntity gongzuoliang, HttpServletRequest request){
        ...
        if(false)
            ...
            gongzuoliang.setJiaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<GongzuoliangEntity> queryWrapper = new EntityWrapper<GongzuoliangEntity>()
            .eq("jiaoshi_id", gongzuoliang.getJiaoshiId())
            .eq("gongzuoliang_name", gongzuoliang.getGongzuoliangName())
            .eq("gongzuoliang_types", gongzuoliang.getGongzuoliangTypes())
            .eq("gongzuoliang_yesno_types", gongzuoliang.getGongzuoliangYesnoTypes())
            .eq("gongzuoliang_yesno_text", gongzuoliang.getGongzuoliangYesnoText())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        GongzuoliangEntity gongzuoliangEntity = gongzuoliangService.selectOne(queryWrapper);
        if(gongzuoliangEntity==null){
            gongzuoliang.setGongzuoliangYesnoTypes(1);
...
//            gongzuoliang.setJiaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<GongzuoliangEntity> queryWrapper = new EntityWrapper<GongzuoliangEntity>()
            ...
            .eq("gongzuoliang_yesno_types", gongzuoliang.getGongzuoliangYesnoTypes())
            .eq("gongzuoliang_yesno_text", gongzuoliang.getGongzuoliangYesnoText())
            .eq("insert_time", gongzuoliang.getInsertTime())
            ;
...
//                            gongzuoliangEntity.setJiaoshiId(Integer.valueOf(data.get(0)));   //教师 要改的
//                            gongzuoliangEntity.setGongzuoliangName(data.get(0));                    //工作量标题 要改的
//                            gongzuoliangEntity.setGongzuoliangTypes(Integer.valueOf(data.get(0)));   //工作量类型 要改的
//                            gongzuoliangEntity.setGongzuoliangContent("");//照片
//                            gongzuoliangEntity.setGongzuoliangNumber(data.get(0));                    //工作量数值 要改的
//                            gongzuoliangEntity.setGongzuoliangYesnoTypes(Integer.valueOf(data.get(0)));   //状态 要改的
//                            gongzuoliangEntity.setGongzuoliangYesnoText(data.get(0));                    //审核意见 要改的
