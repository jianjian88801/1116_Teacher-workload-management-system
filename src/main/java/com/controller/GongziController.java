    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(gongzi !=null){
            ...
            BeanUtils.copyProperties( gongzi , view );//把实体数据重构到view中
                ...
                //级联表
                JiaoshiEntity jiaoshi = jiaoshiService.selectById(gongzi.getJiaoshiId());
                if(jiaoshi != null){
    ...
    */
    ...
    public R save(@RequestBody GongziEntity gongzi, HttpServletRequest request){
        ...
        if(false)
            ...
            gongzi.setJiaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<GongziEntity> queryWrapper = new EntityWrapper<GongziEntity>()
            .eq("jiaoshi_id", gongzi.getJiaoshiId())
            .eq("gongzi_month", gongzi.getGongziMonth())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        GongziEntity gongziEntity = gongziService.selectOne(queryWrapper);
        if(gongziEntity==null){
            gongzi.setShifaMoney(gongzi.getDixinMoney()+gongzi.getJiangjinMoney()+gongzi.getQitaMoney()-gongzi.getWuxianyijinMoney());
            gongzi.setInsertTime(new Date());
            gongzi.setCreateTime(new Date());
            gongziService.insert(gongzi);
            return R.ok();
        }else {
            return R.error(511,"该教师该月份已有工资记录");
        }
    }
...
//                            gongziEntity.setJiaoshiId(Integer.valueOf(data.get(0)));   //教师 要改的
//                            gongziEntity.setGongziName(data.get(0));                    //工资姓名 要改的
//                            gongziEntity.setGongziMonth(data.get(0));                    //月份 要改的
//                            gongziEntity.setDixinMoney(data.get(0));                    //底薪 要改的
//                            gongziEntity.setJiangjinMoney(data.get(0));                    //奖金 要改的
//                            gongziEntity.setQitaMoney(data.get(0));                    //其他 要改的
//                            gongziEntity.setWuxianyijinMoney(data.get(0));                    //五险一金 要改的
//                            gongziEntity.setShifaMoney(data.get(0));                    //实发 要改的
//                            gongziEntity.setBeizhuContent("");//照片
