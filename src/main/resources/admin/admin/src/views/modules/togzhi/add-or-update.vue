    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="jiaoshiId" name="jiaoshiId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="通知标题" prop="togzhiName">
                       <el-input v-model="ruleForm.togzhiName"
                                 placeholder="通知标题" clearable  :readonly="ro.togzhiName"></el-input>
                   </el-form-item>
                   <div v-else>
                       <el-form-item class="input" label="通知标题" prop="togzhiName">
                           <el-input v-model="ruleForm.togzhiName"
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="通知类型" prop="togzhiTypes">
                        <el-select v-model="ruleForm.togzhiTypes" :disabled="ro.togzhiTypes" placeholder="请选择通知类型">
                            <el-option
                                v-for="(item,index) in togzhiTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="通知类型" prop="togzhiValue">
                        <el-input v-model="ruleForm.togzhiValue"
                            placeholder="通知类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="通知内容" prop="togzhiContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.togzhiContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.togzhiContent" label="通知内容" prop="togzhiContent">
                            <span v-html="ruleForm.togzhiContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=togzhi_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.togzhiTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`togzhi/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.togzhiCrossAddOrUpdateFlag = false;
                                        this.parent.search();
